...
def wait_for_device(
        serial, poll_function=find_hid_devices, timeout=LONG_TIMEOUT,
        poll_time=POLL_TIME):
    if timeout <= 0:
...
def get_single_teensy(find_function=find_hid_devices):
    devs = find_function()
    if len(devs) == 0:
...
def soft_reboot_serial(dev=None, find=True):
    if dev is None:
        dev = get_single_teensy(find_serial_devices)
    if isinstance(dev, (str, unicode)):
        dev = find_serial_devices(dev)
...
def program_hid_device(filename, mcu=None, dev=None, autoboot=True):
    ...
    if dev is None:
        dev = get_single_teensy(find_hid_devices)
    if isinstance(dev, (str, unicode)):
        dev = find_hid_devices(dev)
    if dev.is_kernel_driver_active(0):
        dev.detach_kernel_driver(0)
        dev.set_configuration()
...
def boot(mcu=None, dev=None):
    # TODO wait for device to reappear, do HID devices disappear?
    code_size, block_size = get_mcu(mcu)
...
def reset_device(mcu=None, dev=None):
